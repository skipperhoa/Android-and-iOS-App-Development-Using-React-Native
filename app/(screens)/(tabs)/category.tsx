...
export default function CategoryScreen() {
  ...
  const [products, setProducts] = useState([] as any[]); // Khởi tạo mảng sản phẩm rỗng
  const [categories, setCategories] = useState([]);

  /* set pagination load more*/
  const [page, setPage] = useState(1);
  // hasMore là biến để kiểm tra xem còn dữ liệu để load hay không
  const [hasMore, setHasMore] = useState(true);

  useEffect(() => {
    if (params?.title) {
      onLoading();
      init(); // reset lại các giá trị khi search thay đổi
      setSearch(params.title);
    } else {
      getAllCategories().then((res) => {
        if (res.status && res.data?.length) {
          router.setParams({ title: res.data[0].title }); // set params title đầu tiên
        }
      });
    }
  }, [params.title]);
  ...
  useEffect(() => {
    ...
    const fetchData = fetchCategoriesByFilter(search, "id:desc", page);
    Promise.all([fetchData, delay]).then(([res]) => {
      if (res.status) {
        //console.log("first page: ", res.products.last_page, " page: ", page);
        // nếu last_page bằng page thì không còn dữ liệu để load nữa
        if (res.products.last_page === page) {
          setHasMore(false);
        }
        if (res.products.data.length > 0) {
          ...
          setHasMore(false); // nếu không có sản phẩm nào thì không còn dữ liệu để load nữa
        }
  ...
  }, [search, page]); // nếu page hoặc search thay đổi thì sẽ gọi lại api

  const init = () => {
    setPage(1); // reset page về 1 khi search thay đổi
    setProducts([]); // reset mảng sản phẩm khi search thay đổi
    setHasMore(true); // reset hasMore về true khi search thay đổi
  };

  const onLoading = () => {
    Animated.loop(
      Animated.timing(loadAnimatedValue, {
        toValue: 1,
        duration: 1200,
  ...
  const handleLoadMore = () => {
    if (!loading && hasMore) {
      setPage((prevPage) => prevPage + 1); // tăng page lên 1 để load thêm dữ liệu
    }
  };
  ...
  const renderViewLoading = (
    <View className="w-full h-full bg-blue-500/50 flex flex-col justify-center items-center absolute top-0 left-0 z-50">
      <View className="w-full h-full flex flex-col justify-center items-center">
        <Animated.View
          className="w-[45px] h-[45px] flex flex-col justify-center items-center border-[1px] border-blue-600 rounded-full relative z-50"
          style={[
            {
              transform: [
                {
  ...
  /* edit load more */
  const renderViewItem = ({ item }: { item: any }) => {
    return (
      <TouchableOpacity
        ...
        <View className="w-full bg-white rounded-xl pt-5">
          ...
          <View className="flex flex-row items-center justify-between px-5">
            <View className="flex-1 pr-5">
              ...
              <Text
                className="text-xl pt-1 font-bold capitalize"
                numberOfLines={1}
                ellipsizeMode="tail"
              >
                {item.id} - {item.title}
          ...
          <View className="w-full bg-blue-500 p-2 rounded-md mt-2 flex flex-row items-center justify-between gap-2">
            ...
            <View className="w-1/3 text-center">
              ...
              <View className="w-full mt-2">
                <Text className="text-black font-bold text-center">4 chỗ</Text>
              </View>
            </View>
          </View>
        </View>
    ...
    >
      ...
      <View className="w-full">
        <FlatList
          ...
          onEndReachedThreshold={0.5} // scroll đến 50% cuối danh sách sẽ gọi hàm load more
                  ...
                  <View className="w-full flex flex-row gap-1">
                    {categories?.map((item: any) => {
                      return (
                        ...
                        >
                          <Text
                            className="text-sm font-bold p-2 text-[15px]"
                            style={[
                              {
                                color:
                                  params.title == item.title
